...
const TIP_MAP = [
  { tip: 'urun-tedariki',  kelimeler: [
    // Ürün isimleri
    'modem', 'router', 'switch', 'kamera', 'sunucu', 'server', 'bilgisayar',
    'laptop', 'tablet', 'telefon', 'cihaz', 'ekipman', 'malzeme', 'ürün',
    'access point', 'ups', 'nvr', 'dvr', 'kablosu', 'kablo',
    // Teslimat fiilleri — bunlar tek başına da yeterli
    'verildi', 'teslim edildi', 'teslim', 'satıldı', 'temin edildi',
    'temin', 'tedarik edildi', 'tedarik', 'gönderildi', 'gönderilmiş',
    'teslim alındı', 'satış',
  ]},
...
function firmaAdiCikar(text) {
  ...
  // "X Belediyesine / Belediyesi / Belediyede / Belediye" → "X Belediyesi"
  m = text.match(/^([\wÇĞİÖŞÜçğışöü\s\.\-]{2,30}?)\s+belediye[s]?[i]?(?:ne|de|den|ye|nin)?\b/i);
...
function urunAdiCikar(t) {
  // "10 adet <1-3 kelime>" — adet'ten sonraki 1-3 kelimeyi al
  // Dur: fiil / bağlaç / fiyat sözcükleri gelince kes
  const FIIL = /^(?:verildi|teslim|edildi|satıldı|kuruldu|takıldı|gönderildi|ve|ile|fiyatı|fiyat|tl|lira|toplam|birinin|birim)$/i;
  let m = t.match(/\d+\s*adet\s+([\wçğışöüİÇĞŞÜÖ]+(?:\s+[\wçğışöüİÇĞŞÜÖ]+){0,2})/i);
  if (m) {
    // Kelimeleri filtrele
    const kelimeler = m[1].split(/\s+/).filter(k => !FIIL.test(k));
    if (kelimeler.length) {
  ...
  // Bilinen ürün kelimeleri — metin içinde geçiyorsa kullan
  const urunler = [
