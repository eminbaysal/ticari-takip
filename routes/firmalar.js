  ...
  try {
    ...
    hizmetler.forEach(h => {
      ...
      if (isTRY) {
        ...
        else          finMap[fId].kesilmemis += fiyat; // henüz fatura kesilmemiş TRY tutarı
        if (isTahsil) finMap[fId].odeme     += fiyat;
      } else {
        finMap[fId].usdVarMi = true;
        if (!isFatura) finMap[fId].usdKesilmemis += fiyat; // henüz fatura kesilmemiş USD tutarı ($)
    ...
    const result = firmalar.map(f => ({
      ...f.toObject(),
      fin: finMap[f._id.toString()] || { toplam: 0, fatura: 0, odeme: 0, kesilmemis: 0, usdKesilmemis: 0, usdVarMi: false }
    }));
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
